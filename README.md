...
Write-back targets **GitLab** (merge requests — battle-tested in production) or **GitHub** (pull requests — implemented and mock-tested, not yet validated against a live repo; treat as alpha). The provider is auto-detected from the repo URL, or forced via `config.gitProvider`.
